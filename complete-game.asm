...
furl_icon_timer dw 0
show_fuel_icon:db 0;after timer 200 and 340
fuel_col:    dw 0,0,0          ; Column position of fuel (default middle)
...
coin_plus_msg: db '+100', 0  ; +100 followed by ☼ symbol
;------------------------------------------------------------------------------------------------------------------------------------------------------------
;-------------------------------------------------------------------------Helpers---------------------------------------------------------------------------
;---------------------------------------------------------------------------------------------------------------------------------------------------------
...
new_timer_isr:
    push ax
    push bx
	add word[cs:furl_icon_timer],1
    ; Handle lane switch cooldown
    cmp word [cs:lane_switch_cooldown], 0
    je .no_cooldown
...
.no_cooldown:
    ...
    cmp ax, FUEL_TICK_RATE       ; Check if 5 seconds passed
    jb .no_fuel_update
    ...
    cmp al, 0
    je .no_fuel_update           ; Already at 0
    
    sub al, 2                    ; Decrease by 2 cells
    jns .store_fuel              ; If not negative, store it
...
.no_fuel_update:
	cmp word[cs:furl_icon_timer],200
	je show_fuel
	cmp word [cs:furl_icon_timer],350
	je show_fuel
	returnn:
    pop bx
    pop ax
...
crash_effect:
    ...
    ; ─────────────────────────────────────────────────────────────
    ; Display "CRASH!" message in BLACK AREA (LEFT OF ROAD)
    ; ─────────────────────────────────────────────────────────────
    mov dh, 12                  ; Center screen vertically
    mov dl, 2                   ; Black area on left (columns 0-11)
...
coin_consumed_effect:
    ...
    ; Static position for all effects - safe area
    ; Center: row 10, col 20
    
    ; ─────────────────────────────────────────────────────────────
    ; SPARKLE FRAME 1: Initial flash
    ; ─────────────────────────────────────────────────────────────
    mov dh, 10                  ; Static row
    mov dl, 20                  ; Static column
    ...
    mov dh, 11                  ; Bottom-left
    mov dl, 19
    call write_char
    
    mov dl, 21                  ; Bottom-right
    call write_char
    ...
    ; ─────────────────────────────────────────────────────────────
    ; Display "+100" score popup in BLACK AREA (LEFT OF ROAD)
    ; ─────────────────────────────────────────────────────────────
    mov dh, 15                  ; Below sparkle effect
    mov dl, 3                   ; Black area on left (columns 0-11)
    mov bl, 0x0A                ; Bright green on black
    mov si, coin_plus_msg
    call print_string
    
    ; Small delay to show the +100
    call delay_short
    call delay_short
...
scrollLoop:
    ...
    call draw_fuel_popup	; Already shown popup
